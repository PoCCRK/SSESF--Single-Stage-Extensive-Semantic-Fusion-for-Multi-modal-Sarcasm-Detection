...
class SarcasmHandler(TaskHandler):
    def __init__(self, args) -> None:
        ...
        self.supconloss = SupConLoss(temperature=0.07, contrast_mode='all', base_temperature=0.07)
    ...
    def eval_batch(self, model, image, language_tokens, padding_mask, label, image_id):
        logits, outputs = model(
            ...
            padding_mask=padding_mask)
        batch_size = language_tokens.shape[0]
        acc1, = accuracy(logits, label, topk=(1, ))
        self.metric_logger.meters['acc'].update(acc1.item(), n=batch_size)

        # acc = metrics.accuracy_score(label.cpu(), torch.argmax(logits.cpu(), -1))
        # print(acc)
    ...
    def after_eval(self, **kwargs):
        ...
        # return {k: meter.global_avg for k, meter in self.metric_logger.meters.items()}, "acc1"
        acc = self.metric_logger.acc.global_avg
        print(f'acc {acc:.4f}')
        ...
        print(f"acc {acc:4f}", f"precision {precision:4f}", f"recall {recall:4f}", f"f1 {f1:4f}")

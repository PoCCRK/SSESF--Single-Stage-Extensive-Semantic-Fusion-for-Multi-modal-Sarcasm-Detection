...
class SarcasmDataset(BaseDataset):
    ...
    @classmethod
    def make_dataset_index(cls, data_path, tokenizer, image_path, ocr_path):
        ...
        with open(os.path.join(data_path, "train.txt"),'r',encoding='utf-8') as fin:
            ...
            for i in range(len(lines)):
                ...
                if img_id+'.jpg' in filenames:
                    ...
                    #     text_i = text_l[i].strip()
                    #     if random.random() < 0.4:
                    #         text_i = text_i.replace(text_i.split(' ')[0], '<mask>')
                    #     new_text += " # " + text_i

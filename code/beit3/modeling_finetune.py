...
class Sarcasm_Pooler(nn.Module):
...
class Sarcasm_Head(nn.Module):
    def __init__(self, embed_dim, num_classes, norm_layer):
        super().__init__()
        self.pooler = Sarcasm_Pooler(embed_dim, norm_layer)
...
#         )
#         x = outputs["encoder_out"]
#         cls_rep = self.pooler(x, outputs["encoder_padding_mask"].bool())
